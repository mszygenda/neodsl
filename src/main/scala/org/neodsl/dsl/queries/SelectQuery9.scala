 ...
 */
class SelectQuery9[A >: Null <: TypedNode[A], B >: Null <: TypedNode[B], C >: Null <: TypedNode[C], D >: Null <: TypedNode[D], E >: Null <: TypedNode[E], F >: Null <: TypedNode[F], G >: Null <: TypedNode[G], H >: Null <: TypedNode[H], I >: Null <: TypedNode[I]](nodes: List[Node], patterns: PatternComposition, conditions: Condition) extends SelectQuery(nodes, patterns, conditions) {
  def exec[U](processResult: (A, B, C, D, E, F, G, H, I) => U)(implicit engine: ExecutionEngine): Seq[U] = {
    nodes match {
        case nodeA :: nodeB :: nodeC :: nodeD :: nodeE :: nodeF :: nodeG :: nodeH :: nodeI :: Nil => {
        ...
        case _ => {
            throw new Exception("Invalid number of nodes selected for SelectQuery9. It should be 9 nodes")
        }
    }
  }
}
