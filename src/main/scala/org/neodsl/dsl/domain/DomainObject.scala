...
import org.neodsl.queries.domain.{--> => Right, <-- => Left, -- => NoDirection, Relation, TypedNode}
...
abstract class DomainObject[T >: Null <: DomainObject[T]](implicit manifest: Manifest[T]) extends TypedNode[T] {
  ...
  def --[U >: Null <: TypedNode[U]](name: String) = {
    new PatternBuilder[T, U](PatternTriple[T, U](NodePattern(this), RelationPattern(Relation(name, NoDirection), 1 to 1), UndefinedNodePattern[U]))
  }
